...
public class Main {
	...
	public static void addBook(Session session) {
		...
		do {
			...
			title = sc.nextLine();
			if (title.length() == 0)
				System.out.println("Title must have 1 or more symbol,plz try again");
		...
		do {
			...
			name = sc2.nextLine();
			if (name.length() == 0)
				System.out.println("Name must have 1 or more symbol,plz try again");
		} while (name.length() == 0);
	...
	public static void main(String[] args) {
		...
		do {
			...
			if (choice == 1) {
				...
				/*
				 * List<Book> books = (List<Book>)session.createQuery(
				 * "from Book").list(); Collections.sort(books);
				 * Collections.sort(books, new Comparator<Book>(){ public int
				 * compare(Book d1, Book d2){ return
				 * d1.getTitle().compareTo(d2.getTitle()); } });
				 */
				for (Book book : books) {
